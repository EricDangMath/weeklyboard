...
function App() {
  ...
  const visibleEvents = useMemo(() => {
    ...
    if (activeLayer === 'fixed') return [...fixed, ...normalizedEvents.filter((event) => event.layer === 'fixed' && !event.deadline_kind)];
    if (activeLayer === 'flex') return [...normalizedEvents.filter((event) => event.layer === 'flex' && !event.deadline_kind), ...planned];
    return normalizedEvents.filter((event) => event.layer === activeLayer && !event.deadline_kind);
  ...
  const timelineDeadlines = currentWeekDeadlines.filter((item) => !item.layer || item.layer === activeLayer);
  const timelineRange = deadlineTimeRange(timelineDeadlines, DAY_START, DAY_END);
